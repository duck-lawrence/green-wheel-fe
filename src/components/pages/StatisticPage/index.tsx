...
export function StatisticPage({ adminStationId = undefined }: { adminStationId?: string }) {
    const { t } = useTranslation()

    const [stationId, setStationId] = useState<string>(adminStationId || "")

    const { data: stations, isLoading: isStationsLoading } = useGetAllStations({
        enabled: !adminStationId
    })

    useEffect(() => {
        if (adminStationId) return
        if (stations && stations.length > 0 && !stationId) setStationId("")
    }, [adminStationId, stationId, stations])
