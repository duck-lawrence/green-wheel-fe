export const INTRO_PURPOSE = String.raw`These General Terms and Conditions ("GTC") apply to Customers using the car rental services provided by Green Wheel Trading and Service Joint Stock Company ("GW"). In these GTCs, GW and the Customer are each referred to as a "Party" and collectively as the "Parties".`

export const INTRO_DEFINITIONS = String.raw`2.1. "Cost" refers to any amount paid or collected by GW on behalf of the Customer during the rental period, including but not limited to the Rental Fee, Deposit Fee, and Additional Fees (if any).
...
2.10. "Working Hours" refers to the time frame 07:00–17:00 (GMT+7).
...
2.12. "Information Channels" include: (i) GW’s official website and (ii) other digital channels managed by GW.
...
export const SERVICE_DESCRIPTION = String.raw`The service is charged on a daily basis. Each Customer account may only have one (01) active Rental Agreement at a time.`

export const BOOKING_CANCEL = String.raw`2.1. Booking Method:
Customers can only make bookings via the Information Channels or directly at GW stations. GW does not accept bookings made through any other means.
2.2. Booking Time:
...
If a Customer cancels a paid booking or returns the vehicle earlier than the end time stated in the Rental Agreement, all paid amounts are non-refundable under any circumstances, unless the cancellation is due to GW’s fault.`

export const BASE_FEE = String.raw`The Rental Fee reflects the right to use the Vehicle per day. It includes mandatory regulatory charges (if any) and is calculated according to the Rental Agreement.`

export const REMINDER_NOTICE = String.raw`(1) When the Customer returns the Vehicle late, in addition to applicable surcharges, GW will:
...
export const HANDOVER_RETURN = String.raw`- Vehicle handover and return are conducted during Working Hours.
- Both Parties confirm the Vehicle Checklist, verifying the vehicle’s condition, photographs, and relevant readings.`

export const CUSTOMER_RESPONSIBILITY = String.raw`- The Customer shall not use the Vehicle for unlawful purposes; shall not pledge, mortgage, or sublease it; and shall not lend or rent it to third parties.
...
- The Customer must immediately notify GW of any incidents, collisions, or interventions by authorities, and cooperate with the insurer as instructed.
- The Customer shall compensate for all damages caused by their fault (Damage Fee).
- If the Customer fails to remedy a violation within 03 days after receiving notice, GW reserves the right to initiate repossession procedures in accordance with the law.`

export const LIABILITY_LIMIT = String.raw`GW shall only be liable for direct damages resulting from its own fault, up to the total amount paid by the Customer for the corresponding rental period. GW shall not be liable for any indirect, special, consequential, or loss-of-profit damages.`

export const PRIVACY = String.raw`GW processes personal data in accordance with its publicly available Data Policy, serving operational and service improvement purposes. Any Party breaching confidentiality obligations must compensate for all resulting damages.`

export const MISC = String.raw`GW reserves the right to modify or supplement these GTCs and publish updates through its Information Channels. The updated version takes effect from the date of notice.`
