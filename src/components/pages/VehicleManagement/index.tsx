...
type VehicleFilterFormValues = { licensePlate: string; stationId: string | null; status: string | null }
type VehicleEditFormValues   = { licensePlate: string; stationId: string; modelId: string; status: string | null }
...
export function AdminVehicleManagementView() {
  ...
  const { data: vehicleModels = [], isFetching: isFetchingVehicleModels } = useGetAllVehicleModels({ query: {} })
  ...
  const stationNameById = useMemo(
    () => stations.reduce<Record<string, string>>((acc, s) => { acc[s.id] = s.name; return acc }, {}),
    [stations]
  )
...
const statusOptions = useMemo(
  ...
  const filterSchema = useMemo(() => Yup.object({
    licensePlate: Yup.string().trim(),
    stationId:    Yup.string().trim().nullable(),
    status:       Yup.string().nullable()
  }), [])

  const createSchema = useMemo(() => Yup.object({
  ...
  const createFormik = useFormik<CreateVehicleReq>({
    ...
    onSubmit: (v, helpers) => {
      ...
      createMutation.mutate(payload, {
        onSuccess: () => { helpers.resetForm(); createModal.onClose() }
      })
    }
  })
  ...
  const editFormik = useFormik<VehicleEditFormValues>({
    ...
    onSubmit: (values, helpers) => {
      ...
      if (values.stationId && values.stationId !== editingVehicle.stationId) payload.stationId = values.stationId
      if (values.modelId && values.modelId !== editingVehicle.model?.id)     payload.modelId  = values.modelId
      ...
      if (Object.keys(payload).length === 0) {
        helpers.resetForm(); editModal.onClose(); setEditingVehicle(null); return
      }

      updateMutation.mutate({ vehicleId: editingVehicle.id, payload }, {
        onSuccess: () => { helpers.resetForm(); setEditingVehicle(null); editModal.onClose() }
      })
    }
  })
  ...
  const handleConfirmDelete = useCallback(() => {
    ...
    deleteMutation.mutate(deletingVehicle.id, {
      onSuccess: () => { setDeletingVehicle(null); deleteModal.onClose() }
    })
  }, [deleteMutation, deletingVehicle, deleteModal])

  const onStationFilterChange = useCallback((keys: Selection) => {
    if (keys === "all") { filterFormik.setFieldValue("stationId", null); return }
  ...
  const onStatusFilterChange = useCallback((keys: Selection) => {
    if (keys === "all") { filterFormik.setFieldValue("status", null); return }
    const [v] = Array.from(keys)
    filterFormik.setFieldValue("status", v != null ? String(v) : null)
  }, [filterFormik])

  const onOpenEdit = useCallback((v: VehicleWithStatus) => { setEditingVehicle(v); editModal.onOpen() }, [editModal])
  const onCloseEdit = useCallback(() => { editFormik.resetForm(); setEditingVehicle(null); editModal.onClose() }, [editFormik, editModal])

  const onOpenDelete = useCallback((v: VehicleWithStatus) => { setDeletingVehicle(v); deleteModal.onOpen() }, [deleteModal])
  const onCloseDelete = useCallback(() => { setDeletingVehicle(null); deleteModal.onClose() }, [deleteModal])

  return (
    <div className="rounded-3xl border border-slate-200 bg-white px-6 py-8 shadow-sm space-y-6 mb-12">
      {/* Header */}
      <header className="space-y-2">
        <h1 className="text-3xl font-bold text-slate-900">{t("admin.vehicle_management_title")}</h1>
      </header>
        ...
        <form onSubmit={filterFormik.handleSubmit} className="space-y-5">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4 lg:items-end">
            <InputStyled
              ...
              onChange={(e) => filterFormik.setFieldValue("licensePlate", e.target.value)}
            ...
            <FilterTypeStyle
              ...
              selectedKeys={filterFormik.values.stationId ? new Set([filterFormik.values.stationId]) : new Set([])}
              disallowEmptySelection={false}
              isClearable
              onSelectionChange={onStationFilterChange}
            >
              {stationOptions.map((o) => <FilterTypeOption key={o.key}>{o.label}</FilterTypeOption>)}
            </FilterTypeStyle>

            <FilterTypeStyle
              label={t("vehicle.status_label")}
              placeholder={t("vehicle.status_placeholder")}
              selectedKeys={filterFormik.values.status ? new Set([filterFormik.values.status]) : new Set([])}
              disallowEmptySelection={false}
              isClearable
              onSelectionChange={onStatusFilterChange}
            >
              {statusOptions.map((o) => <FilterTypeOption key={o.key}>{o.label}</FilterTypeOption>)}
            </FilterTypeStyle>
      ...
      {totalPages > 1 && (
        <div className="flex justify-center pt-2">
          <PaginationStyled
            page={currentPage}
            total={totalPages}
            onChange={(next: number) => setPagination((p) => ({ ...p, pageNumber: next }))}
            showControls
          />
        </div>
      )}
      ...
      <VehicleCreateModal
        ...
        onClose={() => { createFormik.resetForm(); createModal.onClose() }}
