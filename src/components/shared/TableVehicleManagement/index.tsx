...
function humanizeStatusKey(k?: string) {
  return k
    ? k.toString().toLowerCase().replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())
    : undefined
}
...
export function TableVehicleManagement({
  ...
  const tableRows = useMemo(() => {
    return vehicles.map((vehicle) => {
      ...
      const defaultLabel =
        humanizeStatusKey(statusKey) ?? t("vehicle.status_value_unknown")
      ...
      const stationName = (t("table.unknown_station"), stationNameById[vehicle.stationId] ?? t("table.unknown_station"))
      const brandName =
        (
          resolvedModel &&
          ...
          typeof (resolvedModel as any).brandName === "string"
            ...
            : resolvedModel?.brand?.name
        ) ?? t("table.unknown_brand")
  ...
  }, [onDelete, onEdit, stationNameById, t, vehicles, vehicleModelsById, tUnsafe])
